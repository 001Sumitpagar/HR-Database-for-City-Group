...
select concat(first_name, ' ', last_name) as fullname, salary, department_name
from employees join departments
on employees.department_id = departments.department_id
where employees.manager_id=(select * from employees where first_name ='Steven' and last_name ='king');
...
select department_name,sum(salary)as total_cost 
...
#find the countrywise employee count
select country_name, count(employee_id)  as employee_count from employees
...
-- citywise sum of salary
-- find the employees working more than 10 years in this organization

select first_name, year(current_date())-year(hire_date) as yearsofexp from employees
where year(current_date())-year(hire_date)>10; 

-- which counties has most experienced employees
-- which top 10 employees has delayed joining the organization
...
select employee_id from employees where exists(select employee_id from job_history where job_id='st_clerk');

select employee_id, first_name from employees where exists(select employee_id from job_history where job_id='st_clerk');

-- fetch the employeeid & names of all employe whos last working day in the organization was 1999-12-31

select employee_id, first_name, last_name from employees 
join job_history using (employee_id) where end_date= '1999-12-31';

-- fetch employee id, firstname last name deprtment name & total experiance of all employee who have completed at least 25 years in the organization

select employee_id,first_name, last_name, department_name, year(now())-year(hire_date) as totalexp from employees
join departments using (department_id)
where year(now())-year(hire_date)>=25; 
...
-- fetch the details of employees whose salary is greater than average salaary

select first_name, last_name , salary from employees 
where salary>(select avg(salary) 
from employees);

-- find the names of all employees whose salary is greater than 60% of there department total salary bill

select first_name, last_name, salary from employees 
where salary in (select sum(salary) from employees 
group by department_id having sum(salary)>0.6*sum(salary)) 
